...
criterion_dice.cuda()
criterion_mse.cuda()
data_gener = data_gen.get_voc_detect(batch_size=8, max_detect=max_detect, image_size=[256,256])
...
def run():
    ...
    for x in range(10000000):
        ...
        sem_seg_out = torch.reshape(sem_seg_out,shape=[8,-1])
        seg_mask = torch.reshape(seg_mask,shape=[8,-1])
